...
- 0.5 sparse, 1, evenly spread out, 1.5, clumped around 5 and 6
...
for(i in 1:length(players)){
  ...
  # Use optim() to find the parameter values
  fit<- optim(par=c(0,0), fn=l.likely, x=legs_won, size=total_legs, neg=TRUE)
  # Convert the optimized parameters back to the original scale
  p_estims[i]<- 1/(1 + exp(-fit$par[1]))
  theta_estims[i]<- exp(fit$par[2])
}

# store results in tibble and round vals to 3dp
results <- tibble(
  Player=players,
  p=round(p_estims,3),
...
for(i in 1:length(players)){
  ...
  legs_won <- as.numeric(if_else(matches$Winner==player, matches$WinnerLegs, matches$LoserLegs))
  # fit model
  fit <- optim( par=c(0, 0, 0), fn=l.likely.bb, x=legs_won, neg=TRUE)

  alpha_estims[i]<- exp(fit$par[1])
  beta_estims[i]<- exp(fit$par[2])
  theta_estims[i]<- exp(fit$par[3])
